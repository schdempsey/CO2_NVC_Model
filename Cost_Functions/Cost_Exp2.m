function [f,c,gf,gc] = Cost_DrewBOLD(theta,Data,Con,stimend, FID)
    logL=0;
    ...
    %% Simulations
        solReal = simulate_Drew([0:10:110]',theta, Constants, [], options);
        options.atol = 1e-5;
        options.rtol = 1e-8;
        if (solReal.status<0)
            logL = NaN;
        else
            costCBF = sum((solReal.y(:,5) - Data.Ycbf(4:end,1)).^2./(Data.sigmaYcbf(4:end,1).^2));
            costBOLD = sum((solReal.y(:,6) - Data.Ybold(4:end,1)).^2./(Data.sigmaYbold(4:end,1).^2));
